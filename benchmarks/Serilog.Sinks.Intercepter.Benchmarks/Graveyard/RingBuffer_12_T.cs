...
namespace Serilog.Sinks.Intercepter.Internal.RingBuffer_12_T
{
    internal sealed class RingBuffer_12_T<T>
    {
        /*
        ...
        public RingBuffer_12_T(int capacity)
        {
            if (capacity <= 0)
            {
        ...
        private void AddToSlot(T logEvent, ulong index)
        {
        ...
        public IReadOnlyCollection<T> GetEnumerable()
        {
            return new RingBuffer_12_TEnumerable(this);
        }
        ...
        private class RingBuffer_12_TEnumerable : IReadOnlyCollection<T>
        {
            private readonly RingBuffer_12_T<T> _buffer;

            public RingBuffer_12_TEnumerable(RingBuffer_12_T<T> buffer)
            {
                _buffer = buffer;
            }
            ...
            {
                return new RingBuffer_12_TEnumerator(_buffer);
            }
        ...
        private class RingBuffer_12_TEnumerator : IEnumerator<T>
        {
            private readonly RingBuffer_12_T<T> _RingBuffer_12_T;
            private int _index = -1;

            public RingBuffer_12_TEnumerator(RingBuffer_12_T<T> RingBuffer_12_T)
            {
                _RingBuffer_12_T = RingBuffer_12_T;
                var currentIndex = (int)(_RingBuffer_12_T._amountAdded & unchecked(COMPLETE_ADDING_MASK - 1));
                if (currentIndex < RingBuffer_12_T.Capacity)
                {
                    _index = -1;
                }
                else
                {
                    _index = currentIndex - RingBuffer_12_T._slots.Length - 1;
                }
            }
            ...
            public T Current
            {
                get
                {
                    var index = _index % _RingBuffer_12_T._slots.Length;

                    ref var slot = ref _RingBuffer_12_T._slots[index];
            ...
            public bool MoveNext()
            {
                ...
                var currentIndex = _RingBuffer_12_T._amountAdded & unchecked(COMPLETE_ADDING_MASK - 1);
