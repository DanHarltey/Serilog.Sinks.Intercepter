...
public sealed class LogLevelBufferIntercepter2 : IIntercepter
{
    private readonly LogEventLevel _triggerLevel;
    private volatile BlockingCollection<LogEvent>? _buffer = new();

    public LogLevelBufferIntercepter2(LogEventLevel triggerLevel) => _triggerLevel = triggerLevel;
    ...
    private static IEnumerable<LogEvent> BufferAlreadyFlushed(LogEvent logEvent) => new Single2( logEvent);
}